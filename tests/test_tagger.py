...
def test_verbatim_validator_accepts_exact_substring():
    is_valid, _ = validate_quote("export it to Excel", "Every Monday I export it to Excel and clean it by hand")
    assert is_valid
...
def test_chunks_of_80_utterances_with_10_overlap():
    utterances = [{"idx": i, "text": f"utterance {i}", "speaker": "A", "side": "us"} for i in range(200)]
...
def test_highlights_deduped_on_utterance_and_tag_across_chunks():
    ...
    highlights = [
        TaggerHighlight(utterance_idx=5, tag_key="pain", quote="test", confidence=0.8, rationale=""),
        TaggerHighlight(utterance_idx=5, tag_key="pain", quote="test", confidence=0.9, rationale=""),
        TaggerHighlight(utterance_idx=5, tag_key="workaround", quote="test", confidence=0.7, rationale=""),
    ]
...
async def test_tagger_persists_suggested_highlights(
    ...
    async with session_factory() as db:
        ...
        for i, (speaker, side, text) in enumerate([
            ...
            ("Maria", "them", "right now we have this spreadsheet where the team logs every infringement they find manually"),
...
async def test_unknown_tag_key_from_llm_is_dropped(
    ...
    async with session_factory() as db:
        ...
        db.add(Utterance(
            ...
            speaker_label="David", speaker_side="us", text="test utterance",
        ))
        await db.flush()
        convo_id = convo.id
...
async def test_fabricated_quote_is_dropped_with_warning(
    ...
    async with session_factory() as db:
        ...
        db.add(Utterance(
            conversation_id=convo.id, idx=0,
            speaker_label="Maria", speaker_side="them",
